...
class DataModuleForMIMIC(pl.LightningDataModule):
    def __init__(self, args):
        ...
        self.args = args
        self.tokenizer = CLIPProcessor.from_pretrained(model_path).tokenizer
        self.image_processor = CLIPProcessor.from_pretrained(model_path).feature_extractor
    ...
    def setup_dataset_for_mention(self, data):
        ...
        input_data = []
        for sample_dict in tqdm(data, desc='Mention PreProcessing'):
            sample_type = 1
    ...
    def train_collator(self, samples):
        ...
        for sample_idx, sample in enumerate(samples):
            ...
            sample_type.append(sample.pop('sample_type'))   # input type: 1 for mention and 0 for entity
        ...
        # for the entity information, we use prefix 'ent_' to tag them,每一个key前面都有一个ent_
