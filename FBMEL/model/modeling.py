...
class MIMICEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.image_feature_fc = nn.Linear(3072, 512)
        self.image_tokens_fc = nn.Linear(self.args.model.input_image_hidden_dim, self.args.model.dv)
        self.layer_norm1 = nn.LayerNorm(3072)

    def get_intermediate_layer_output(module, input, output):
    ...
    def forward(self,
        ...
        image_hidden_states_4 = clip_output.vision_model_output.hidden_states[4][:, 0,:]
        image_hidden_states_10 = clip_output.vision_model_output.hidden_states[10][:, 0,:]
        image_hidden_states_11 = clip_output.vision_model_output.hidden_states[11][:, 0,:]
        image_hidden_states_12 = clip_output.vision_model_output.hidden_states[12][:, 0,:]
        image_hidden_states = torch.cat((image_hidden_states_4, image_hidden_states_10, image_hidden_states_11, image_hidden_states_12),dim=-1)



        image_hidden_states_1 = self.layer_norm1(image_hidden_states)
        image_feature = self.image_feature_fc(image_hidden_states_1)


        text_seq_tokens = clip_output.text_model_output[0]
        image_patch_tokens = clip_output.vision_model_output[0]

        image_embeds = self.image_cls_fc(image_feature)
        image_patch_tokens = self.image_tokens_fc(image_patch_tokens) #  torch.Size([128, 50, 768])
        # print('image_patch_tokens:', image_patch_tokens.shape) torch.Size([128, 50, 96])
